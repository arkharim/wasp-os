...
class AnalogBresnChelloClock():
    """Analog clock application."""

    NAME = 'A.BresnChello Clock'
    #TODO New icon
    ...
    def _update_hand_bresenham(self, time, dial_number, x_center, y_center, hand, on_screen_hand, color):
        ...
        # print("p1y ", p1y)
        # print("p2y ", p2y)
        self.plotLineChello(color, p1x, p1y, p2x, p2y, 1)  # Width = 1

    """Funciona pero se debe optimizar el fill. Es muy complicado optimizarlo para las x y las y a la vez."""
    def plotLineChello(self, color, x0, y0, x1, y1, wd):
        draw = wasp.watch.drawable
        ...
        buf = memoryview(display.linebuffer)[0:2 * wd]
        draw565._fill(buf, color, 1, 0)
        display.quick_start()
        ...
        while True: # / * loop * /
            ...
            # draw.fill(color, x0 - wd // 2, y0 - wd // 2, wd, wd)
            # endtime = time.process_time()
            # fill_time += endtime - starttime
            #Rawblit mucho mas lento que el fill. y no sale bien.
            # display.rawblit(buf, x0 - wd // 2, y0 - wd // 2, wd, wd)
            # set_window(x0 - wd // 2, y0 - wd // 2, wd, wd)
            # quick_write(buf)
